#######main file for adding the treeview table contents for all treeview
...
options2.set("")
t1 = OptionMenu(my_w, options,"Three","Four","Five")
t1.grid(row=3, column=2)

opt1 = OptionMenu(my_w, options1, "Three", "Four", "Five")
...
# add one text box
t3 = OptionMenu(my_w, options2, "Three", "Four", "Five")
...
def add_data():
    my_name = options1.get()  # read name
    my_class = options.get()  # read class
    my_mark = options2.get()  # read mark
    my_gender = options.get()  # read gender
    global i
    i = i + 1
    trv.insert("", 'end',
               values=(i-1, my_name, my_class, my_mark, my_gender))
